...
for i = 1:length(x)
    if ~isempty(Channels_name{i}) %Check channel name
        ...
        if x(i) == Xmax %At right edge of plot
            switch y(i)
                case Ymax
                    text(x(i) - 60, y(i) - 30, Channels_name{i}(IndSlice), 'FontSize', 15) % Top
                case Ymin
                    text(x(i) - 60, y(i) + 25, Channels_name{i}(IndSlice), 'FontSize', 15) % Bottom
                otherwise
                    text(x(i) - 60, y(i) + 3, Channels_name{i}(IndSlice), 'FontSize', 15) % Center
            end
        end
        if y(i) == Ymax %At top
            switch x(i)
                ...
                case Xmin
                    text(x(i) + 10, y(i) - 30, Channels_name{i}(IndSlice), 'FontSize', 15) % Left
                otherwise
                    text(x(i) - 20, y(i) - 30, Channels_name{i}(IndSlice), 'FontSize', 15) % Center
            end
        end
        if x(i) == Xmin %At left edge of plot
            switch y(i)
                ...
                case Ymin
                    text(x(i) + 10, y(i) + 30, Channels_name{i}(IndSlice), 'FontSize', 15) % Bottom
                otherwise
                    text(x(i) + 20, y(i) + 3, Channels_name{i}(IndSlice), 'FontSize', 15) % Center
            end
        end
        ...
        if (x(i) > Xmin && x(i) < Xmax) && (y(i) < Ymax && y(i) > Ymin) %Center points
            text(x(i) - 20 , y(i) + 30, Channels_name{i}(IndSlice), 'FontSize', 15)
        end
